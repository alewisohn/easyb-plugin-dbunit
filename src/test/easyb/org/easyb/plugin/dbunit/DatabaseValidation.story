...
scenario "using database_model call with string values, etc", {
    ...
    given "a database is up and running", {
        ...
        Server.main(["-port", "5500", "-no_system_exit", "true"] as String[])
    }

    and "given the database has an initalized data model", {
        sql = Sql.newInstance("jdbc:hsqldb:hsql://127.0.0.1:5500",
    ...
    and "given the database_model method is invoked with a dataset", {
		...
		database_model("org.hsqldb.jdbcDriver",
                "jdbc:hsqldb:hsql://127.0.0.1:5500", "sa", "", config) {
...
scenario "using database_model call with a live connection", {
	...
	when "a select statement is issued for a value that could only be present if the database_model call worked", {
		value = null
		isql = Sql.newInstance("jdbc:hsqldb:hsql://127.0.0.1:5500",
				"sa", "", "org.hsqldb.jdbcDriver")
